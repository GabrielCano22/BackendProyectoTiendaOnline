...
class Cliente(Usuario):
    ...
    def obtener_info(self) -> str:
        return f"Cliente: {self.nombre} | Compras realizadas: {len(self.historial)}"
...
class Administrador(Usuario):
    ...
    def registrar_producto(self, tienda: Tienda) -> None:
        ...
        if categoria == "1":
            categoria_alimento: str = input(
                "  Categoría (ej: Granos, Lácteos): "
            ).strip()
            nuevo = Alimentos(codigo, nombre, marca, precio, stock, categoria_alimento)

        elif categoria == "2":
            tipo_producto: str = input(
                "  Tipo (ej: Televisor, Celular, Computadora): "
            ).strip()
            nuevo = ProductosElectronicos(
                codigo, nombre, marca, precio, stock, tipo_producto
            )

        elif categoria == "3":
            tipo_cuidado: str = input("  Tipo (ej: Cabello, Facial): ").strip()
            genero: str = input("  Género (Hombre, Mujer, Unisex): ").strip()
            nuevo = CuidadoPersonal(
                codigo, nombre, marca, precio, stock, tipo_cuidado, genero
            )
            ...
            categoria_bebe: str = input(
                "  Categoría (ej: Higiene, Nutrición, Ropa): "
            ).strip()
            rango_edad: str = input("  Rango de edad (ej: 0-6 meses): ").strip()
            nuevo = Bebes(
                codigo, nombre, marca, precio, stock, categoria_bebe, rango_edad
            )

        elif categoria == "7":
            categoria_hogar: str = input(
                "  Categoría (ej: Decoración, Muebles): "
            ).strip()
            habitacion: str = input(
                "  Habitación (ej: Sala, Cocina, Dormitorio): "
            ).strip()
            nuevo = Hogar(
                codigo, nombre, marca, precio, stock, categoria_hogar, habitacion
            )
